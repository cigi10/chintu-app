...
export default function StudyRooms() {
  ...
  function toggleTag(tagId) {
    ...
    if (activeTag) {
      const prev = channelsRef.current[activeTag];
      if (prev) prev.untrack();
    }
          ...
          {TAGS.map((tag) => {
            ...
            return (
              <button
                key={tag.id}
                type="button"
                className={`rooms__tag${
                  isActive ? " rooms__tag--active" : ""
                }`}
              ...
              >
                ...
                {count > 0 && (
                  <span className="rooms__tag-count">{count}</span>
                )}
              </button>
            );
          })}
