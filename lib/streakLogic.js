...
function load() {
  try {
    ...
    // Always a fresh object — callers mutate the result in place
    // (data.studyDates.push(...)), so returning a shared reference here
    // would let that mutation leak into every other call.
    return raw ? JSON.parse(raw) : { ...DEFAULT_STREAK, studyDates: [], resetLog: [] };
  } catch { return { ...DEFAULT_STREAK, studyDates: [], resetLog: [] }; }
}
function save(data) {
